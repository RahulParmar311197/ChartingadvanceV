...
async function loadCandles(symbol, interval, from, to) {
  if (API_URL) {
    ...
    try {
      ...
      console.warn("Market API unavailable; using explicitly simulated demo provider.", error);
    }
  }
...
function Chart({ symbol, interval }) {
  ...
  useEffect(() => {
    ...
    loadCandles(symbol, interval, from, to).then((candles) => {
      ...
      volume.setData(candles.map((c) => ({ time: c.time, value: c.volume ?? 0, color: c.close >= c.open ? "rgba(38,166,154,.45)" : "rgba(239,83,80,.45)" })));
    ...
    <div className="app">
      ...
      <main className="workspace">
        ...
        <section className="chart-area">
          <div className="chart-header"><div><strong>{symbol.symbol}</strong><span className="muted"> · {interval}</span><span className="status-dot"/><span className="muted">{API_URL ? "API / demo fallback" : "Demo data"}</span></div><div className="ohlc"><span>O {symbol.price.toFixed(2)}</span><span>H {(symbol.price*1.014).toFixed(2)}</span><span>L {(symbol.price*.987).toFixed(2)}</span><span>C {symbol.price.toFixed(2)}</span><span className={symbol.change >= 0 ? "positive" : "negative"}>{symbol.change >= 0 ? "+" : ""}{symbol.change.toFixed(2)}%</span></div></div>
