...
function makeData(seed = 100) {
  let value = seed;
  const rows = [];
  for (let i = 0; i < 180; i += 1) {
    const open = value;
    const drift = Math.sin(i / 8) * 0.8 + (Math.random() - 0.48) * 2.5;
    const close = Math.max(1, open + drift);
    rows.push({
      time: Math.floor(new Date(2026, 0, 1, 9, 30).getTime() / 1000) + i * 3600,
      open,
      high: Math.max(open, close) + Math.random() * 2.2,
      low: Math.min(open, close) - Math.random() * 2.2,
      close
    });
    value = close;
  }
...
function Chart({ symbol, interval }) {
  ...
  useEffect(() => {
    ...
    const candles = makeData(150 + symbol.length);
    series.setData(candles);
    const volume = chart.addSeries(HistogramSeries, {
      priceFormat: { type: "volume" }, priceScaleId: "",
      scaleMargins: { top: 0.82, bottom: 0 }
    });
    volume.setData(makeData(70).map((d, i) => ({
      time: d.time, value: 20 + Math.abs(d.close - d.open) * 30 + (i % 9) * 4,
    ...
    <div className="app">
      ...
      <main className="workspace">
        ...
        <section className="chart-area">
          <div className="chart-header"><div><strong>{symbol.symbol}</strong><span className="muted"> · {interval}</span><span className="status-dot"/><span className="muted">Market open</span></div><div className="ohlc"><span>O {symbol.price.toFixed(2)}</span><span>H {(symbol.price*1.014).toFixed(2)}</span><span>L {(symbol.price*.987).toFixed(2)}</span><span>C {symbol.price.toFixed(2)}</span><span className={symbol.change >= 0 ? "positive" : "negative"}>{symbol.change >= 0 ? "+" : ""}{symbol.change.toFixed(2)}%</span></div></div>
        ...
        <aside className="right-panel">
          ...
          <div className="news"><div className="panel-tabs"><strong>News</strong><button className="text-btn">All</button></div>{["Markets open higher as tech leads gains","AI infrastructure stocks remain in focus","Dollar index retreats from recent highs"].map((n,i)=><article key={i}><small>2h ago · Market News</small><p>{n}</p></article>)}</div>
        </aside>
      </main>
      <footer className="bottom-bar">{["Stock Screener","Pine Editor","Strategy Tester","Replay Trading","Paper Trading","Trading Panel"].map((x)=><button key={x} className={bottom===x?"bottom-tab active":"bottom-tab"} onClick={()=>setBottom(x)}>{x}</button>)}<div className="footer-spacer"/><span className="connection"><span className="status-dot"/> Data connection</span></footer>
