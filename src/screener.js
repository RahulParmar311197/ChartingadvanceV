...
export async function fetchFundamentals({ apiUrl, filters = [], groups = [], symbols = [], limit = 25, cursor, signal } = {}) {
  ...
  const response = await fetch(`${apiUrl.replace(/\/$/, "")}/v1/screener/fundamentals?${params}`, { signal });
