...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/v1/workspace" && req.method === "GET") {
      if (!workspaceService) return json(res, 200, { data: getWorkspace(userId(req)), meta: workspaceMeta(false) });
      const document = await workspaceService.getDocument(userId(req));
      return json(res, 200, { data: document.state, meta: { ...workspaceMeta(true), revision: document.revision, updatedAt: document.updatedAt } }, { etag: `"${document.revision}"` });
    }
    if (url.pathname === "/v1/workspace" && req.method === "PUT") {
      try {
        const raw = await readBody(req);
        if (!raw.length) return json(res, 400, { error: { code: "INVALID_BODY", message: "JSON body is required" } });
        ...
        if (!workspaceService) return json(res, 200, { data: saveWorkspace(userId(req), patch), meta: workspaceMeta(false) });
        ...
        return json(res, status, { error: { code, message: code === "BODY_TOO_LARGE" ? "Request body exceeds 32 KiB" : code === "INVALID_WORKSPACE_REVISION" ? "Workspace revision must be a non-negative integer" : code === "WORKSPACE_REVISION_CONFLICT" ? "Workspace was modified by another request; reload and retry" : "Malformed JSON body" } });
      }
    }
