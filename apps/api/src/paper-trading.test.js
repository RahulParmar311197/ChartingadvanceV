...
describe("paper trading application service", () => {
  it("fills a valid market order and updates the portfolio ledger", () => {
    const result = submitPaperOrder("user-a", { id: "order-1", symbolId: "NASDAQ:AAPL", side: "buy", type: "market", quantity: 10 }, 1_000);
  ...
  it("marks filled positions to the deterministic demo quote", () => {
    submitPaperOrder("user-a", { id: "mark", symbolId: "NASDAQ:AAPL", side: "buy", type: "market", quantity: 2 }, 1_000);
    const portfolio = getPaperPortfolio("user-a");
    expect(portfolio.positions[0].unrealizedPnl).toBeTypeOf("number");
    expect(portfolio.account.equity).toBeTypeOf("number");
  });
  ...
  it("does not fill an untriggered limit order and allows cancellation", () => {
    ...
    const cancelled = cancelPaperOrder("user-a", "limit", 2_000);
    expect(cancelled.cancelled).toBe(true);
    expect(cancelled.order.status).toBe("cancelled");
    expect(getPaperAudit("user-a").map((event) => event.action)).toEqual(["order_submitted", "order_accepted", "order_cancelled"]);
  });
  ...
  it("rejects cancellation of a filled order without mutating the fill", () => {
    ...
    expect(() => cancelPaperOrder("user-a", "filled", 2_000)).toThrow("open paper order not found");
    expect(getPaperPortfolio("user-a").ledger).toHaveLength(1);
  });

  it("replaces an accepted order with a new lifecycle id", () => {
    ...
    expect(() => cancelPaperOrder("user-a", "old", 3_000)).toThrow("open paper order not found");
    expect(getPaperAudit("user-a").map((event) => event.action)).toEqual(["order_submitted", "order_accepted", "order_cancelled", "order_replaced", "order_submitted", "order_accepted"]);
  });

  it("prevents replacing a filled order and preserves account state", () => {
    submitPaperOrder("user-a", { id: "filled", symbolId: "NASDAQ:AAPL", side: "buy", type: "market", quantity: 1 }, 1_000);
    expect(() => replacePaperOrder("user-a", "filled", { id: "new" }, 2_000)).toThrow("open paper order not found");
