...
export function applyFillToPortfolio(portfolio: PaperPortfolio, fill: Fill, side: OrderSide): PaperPortfolio {
  if (!fill.orderId || !finite(fill.quantity) || fill.quantity <= 0 || !finite(fill.price) || fill.price <= 0 || !finite(fill.fee) || fill.fee < 0) throw new Error('invalid fill');
  const existing = portfolio.ledger.some((entry) => entry.referenceId === fill.id);
  if (existing) return portfolio;
  const positionIndex = portfolio.positions.findIndex((position) => position.symbolId === portfolio.positions.find((p) => p.symbolId)?.symbolId);
  ...
  const nextPosition: Position = position
    ...
    : { accountId: portfolio.account.id, symbolId: '', quantity: signed, averagePrice: fill.price, realizedPnl: 0, unrealizedPnl: 0 };
